...
infect.range = matrix(0, ncol(results.1x), 2)
...
# load results of uncertainty analysis:
results.2x <- readMat("results2x_n.mat")$results

# find min and max for each coverage level
infect.range = matrix(0, ncol(results.2x), 2)
...
plot.title = c("2 treatment rounds")
...
# load results of uncertainty analysis:
results.3x <- readMat("results3x_n.mat")$results

# find min and max for each coverage level
infect.range = matrix(0, ncol(results.3x), 2)
for (i in 1:ncol(results.3x)){
