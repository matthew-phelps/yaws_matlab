...
people_infect = zeros(loops, length(coverage), length(time));
percent_infect_pre = zeros(loops, length(coverage),length(time));
...
for j = 1:length(coverage) % iterate over the different coverage levels defined by the 'coverage variable
    ...
    for i = 1:loops
        ...
        percent_infect_pre (i, j) = ((E(699)+IP1(end)+ IP2(699) + L1(end) + L2(699) + IS(699)) / N(699));
    end;
...
prev_post_std = zeros(length(time),length(coverage));
prev_pre_mean = zeros(length(time),length(coverage));
prev_pre_std = zeros(length(time),length(coverage));

for j = 1:length(coverage);
    prev_post_mean(j) = mean(results(:,j));
    prev_post_std(j) = std(results(:,j));
    prev_pre_mean(j) = mean(percent_infect_pre(:,j));
    prev_pre_std(j) = std(percent_infect_pre(:,j));
    
end;

save('x2_TCT_prev_post_mean_reduction.mat', 'prev_post_mean')
save('x2_TCT_prev_post_std_reduction.mat', 'prev_post_std')
save('x2_TCT_prev_pre_mean_reduction.mat', 'prev_pre_mean')
save('x2_TCT_prev_pre_std_reduction.mat', 'prev_pre_std')
...
%save results1x_Pr_newCovApproach.mat
