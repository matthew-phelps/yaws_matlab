...
step_betaONE = 0.1*betaONEvect(1);
...
for i = 1:loops2 
    if find(Aa == i) > 0    %sequence, administrative thing, making sure that only 1 variable is moved at a time
        ...
        if gammavect(i+1) <= 0 || gammavect(i+1) > 1; %
    ...
    if find(Bb == i) > 0   
        ...
        if epsilonvect(i+1) < 0 || epsilonvect(i+1) > 1; 
   ...
   if find(Cc == i) > 0    
        ...
        if etavect(i+1) <= 0 || etavect(i+1) > 1; 
   ...
   %% Theta
    ...
    if find(Dd == i) > 0    
        ...
        if thetavect(i+1) <= 0 || thetavect(i+1) >1 ; %get more specific numbers
   ...
   %% Lambda
    ...
    if find(Ee == i) > 0    
        ...
        if lambdavect(i+1) <= 0 || lambdavect(i+1) > 1; 
    ...
    if find(Ff == i) > 0    
        ...
        if betaONEvect(i+1) <= 0 || betaONEvect(i+1) > 1; %0.0002, 0.002
