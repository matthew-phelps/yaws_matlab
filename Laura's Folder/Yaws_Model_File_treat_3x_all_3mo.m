...
for j = 1:length(coverage) % iterate over the different coverage levels defined by the 'coverage variable
    ...
    timeSteps3 = zeros(1,4);
    
    timeSteps4 = zeros(1,2);
    
    timeSteps5 = zeros(1,4);
    ...
    [t2,X2]= ode15s('Yaws_ODE_FileV2',timeSteps2,X1(end,:));
    ...
    [t3,X3]= ode15s('Yaws_ODE_FileV2',timeSteps3,X2(end,:));
    ...
    [t4,X4]= ode15s('Yaws_ODE_FileV2',timeSteps4,X3(end,:));
    
    %% 3 month no treatment - 2nd
    ...
    [t6,X6]= ode15s('Yaws_ODE_FileV2',timeSteps6,X5(end,:));
    ...
    PE_3x(j) = (E(end)+IP1(end)+ IP2(end) + L1(end) + L2(end) + IS(end)) / N(end);
